...
def get_train_args_parser():
    ...
    parser.add_argument("--dataset", default="faust", choices=['faust', 'smal-r', 'shrec20', 'topkids'], help="the dataset to train/test on")
    ...
    parser.add_argument("--fourier", type=int, default=0, help="number of random Fourier feature frequencies appended to the data vector (n_ff)")
    ...
    parser.add_argument("--resample", type=int, default=0, help="number of samples for each epoch, 0 equals no resampling (Section 4.6.1)")
    ...
    parser.add_argument("--resample-p", default=1.0, type=float, help="percentage of the resampled points that come from new random face locations as opposed to the original point set")
...
def validate_train_args(args):
    ...
    if args.dataset in ("shrec20", "smal-r", "topkids") and not args.sparse:
